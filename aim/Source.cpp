...
#define dwLocalPlayer 0xD8D2CC
#define dwEntityList 0x4DA521C
#define dwViewMatrix 0x4D96B34
...
int main() {
	...
	while (!GetAsyncKeyState(VK_END)) { //press the "end" key to end the hack
		...
		if (GetAsyncKeyState(VK_CONTROL /*alt key*/) && closestw2shead.z >= 0.001f /*onscreen check*/)
